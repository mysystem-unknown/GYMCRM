...
export function LoginView() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Login succeeded — update store directly (no page reload needed)
      console.log('[Login] Success, updating store:', data.user?.email, data.user?.role);
      setUser(data.user);
      ...
      if (data.user?.role === 'super_admin') {
        try {
          ...
          console.error('[Login] Failed to fetch gym list:', err);
        }
      }
